...
class SlakhStemMixDataset(SlakhDataset):
    ...
    def _build_dataset(self, root_dir, shuffle=True):
        ...
        # NOTE: load from a pre-saved json file if exists, for reproducibility
        stem_mix_file = os.path.join(
            os.path.dirname(os.path.realpath(__file__)), "stem_mix_train.json"
        )
        if os.path.exists(stem_mix_file):
            print(f"Loading stem-mix from pre-saved json file: {stem_mix_file}...")
            df = json.load(open(stem_mix_file))
        else:
            print("Producing stem-mix dataset...")
            for idx, a_f in enumerate(audio_files):
                # get path for stems
                ...
                # NOTE: following MT3 appendix B.2, choose 10 random subsets of at least 4 instruments
                # these subsets are fixed across all experiments, i.e. they are not randomly sampled on-the-fly
                for num_subset in range(10):
                    if num_subset == 0:
                        # for the first one, always include all the stems
                        dic = {'inst_names': inst_names, 'audio_path': stem_audio_files, 'midi_path': stem_midi_files}
                        df.append(dic)
                    else:
                        # for the rest, randomly select 4 to 8 stems
                        num_stems = random.randint(4, len(inst_names) - 1)
                        chosen_audio_stems = sorted(random.sample(stem_audio_files, k=num_stems))
                        chosen_midi_stems = [k.replace(self.stems_folder, self.midi_folder).replace(".flac", ".mid") for k in chosen_audio_stems]
                        dic = {'inst_names': inst_names, 'audio_path': chosen_audio_stems, 'midi_path': chosen_midi_stems}
                        df.append(dic)
            
            # save the dataset to json file
            with open("dataset/stem_mix_train.json", "w") as f:
    ...
    def _random_stem_mix(self, row):        
        # NOTE: no sampling here anymore, we use all the stems pre-sampled in _build_dataset
        chosen_audio_stems = row['audio_path']
        chosen_midi_stems = [k.replace(self.stems_folder, self.midi_folder).replace(".flac", ".mid") for k in chosen_audio_stems]
